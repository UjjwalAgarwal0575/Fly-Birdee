...
void text::init(SDL_Renderer *rend,SDL_Window *wind)
{
    ...
    surfacemenu[3] = TTF_RenderText_Solid(gFont1, label[3], color);
    //surfacemenu[4] = TTF_RenderText_Solid(gFont1, label[4], {255,0,0,0});
    surfacemenu[4] = TTF_RenderText_Solid(gFont1, label[4], color);
    surfacemenu[5] = TTF_RenderText_Solid(f, label[5], color);
    //surfacemenu[6] = TTF_RenderText_Solid(gFont1, label[6], {0,0,255,0});
    ...
    //SDL_Texture *t_6 = SDL_CreateTextureFromSurface(rend, surfacemenu[6]);
		...
		while(g->running()){
			...
			flag=g->Checkcollision(inputText);
                  	if(!flag){
                    	...
                    	SDL_Rect r={SCREEN_WIDTH-60,0,sur->w,sur->h};
			...
			last = first;
               ...
               init(rend,wind);
               break;
                // SDL_RenderCopy(rend,t_6,NULL,&rect[6]);
	...
	if((e.type == SDL_MOUSEBUTTONDOWN) && (e.motion.x > rect[1].x) && (e.motion.x < (rect[1].x + rect[1].w)) && (e.motion.y > rect[1].y) && (e.motion.y < (rect[1].y + rect[1].h))){
		//SDL_Window *window = SDL_CreateWindow("title", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_RESIZABLE);
		...
		for(int i=0;i<v.size();i++){
			...
			//string str(std::to_string(v[i].second));
        ...
        //SDL_RenderCopy(rend,t_3,NULL,&rect[3]);
        SDL_RenderCopy(rend,t_4,NULL,&rect[4]);
        SDL_RenderCopy(rend,t_5,NULL,&rect[5]);
